...
def plot_column_statistics(df, columns):
    ...
    for ax, col in zip(axes, columns):
        if col in df.columns:
            ...
            ax.bar(stats.keys(), stats.values(), color=['skyblue', 'orange', 'lightgreen', 'salmon'])
...
def plot_donut_win(lol_data):

    outcome_counts = lol_data['win'].value_counts()
    labels = ['Win', 'Loss']
    values = [outcome_counts.get(True, 0), outcome_counts.get(False, 0)]
    colors = ['mediumseagreen', 'tomato']

    fig, ax = plt.subplots(figsize=(8,8))
    ax.pie(values, labels=labels, autopct='%1.1f%%', startangle=90,
           colors=colors, wedgeprops=dict(width=0.3, edgecolor='w'))
    ax.set_title("Distribuzione Win/Loss", fontsize=16)
    plt.show()
...
def classification_report_from_string(report_str):
    ...
    report_dict = {}
    for line in lines[2:4]:  # assumendo due classi: 0 e 1
        if line.strip():
            parts = line.split()
            class_name = parts[0]
...
def plot_classification_report(report_str, model_name):
    ...
    axes[0].set_title('Metriche - Sconfitta (classe 0)')

    axes[1].bar(names, values_1, color=['blue', 'cyan', 'purple'])
    axes[1].set_ylim(0, 1)
    axes[1].set_ylabel('Score')
    axes[1].set_title('Metriche - Vittoria (classe 1)')

    fig.suptitle(f"Report per {model_name}")
    plt.tight_layout()
    plt.show()
...
def plot_learning_curves(model, x, y, model_name):
    ...
    for i, scoring in enumerate(scoring_metrics):
        train_sizes, train_scores, test_scores = learning_curve(
            model, x, y, cv=10, scoring=scoring, n_jobs=-1
        )
        ...
        plt.fill_between(train_sizes_percent, train_errors - train_std, train_errors + train_std,
                         color=colors[i][1], alpha=0.3)
        plt.plot(train_sizes_percent, test_errors, label="Test error", color=colors[i][0], linestyle='dashed')
        plt.fill_between(train_sizes_percent, test_errors - test_std, test_errors + test_std,
                         color=colors[i][1], alpha=0.3)
        ...
        table = plt.table(
            cellText=cell_text,
            colLabels=['Training Size (%)', 'Train Error', 'Test Error'],
            loc='center'
        )
